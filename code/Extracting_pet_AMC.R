...
# 2) NetCDF data of Potential Evapotranspiration from CRU
#
...
# Unzip
setwd(data_ncdf_dir)
fileinput <- list.files(pattern = "cru_ts4.05.1901.2020.pet.dat.nc")
fileouput <- file.path(tmp_dir, "potentialevapotranspiration.nc")
...
for (i in 27:nl) {
  ...
  df <- data.frame(munic, amc_1980, pet, date)

  # rename variables
  colnames(df)[3] <- "monthly_pet"

  # save data as .csv
  setwd(out_dir)
  write.csv(df,
    paste0(out_dir, "/amc_pet_csv/", date, "_amc_pet.csv"),
    row.names = TRUE,
  ) # overwrites
