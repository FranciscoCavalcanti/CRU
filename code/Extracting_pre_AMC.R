...
# Unzip
setwd(data_ncdf_dir)
fileinput <- list.files(pattern = "cru_ts4.05.1901.2020.pre.dat.nc")
fileouput <- file.path(tmp_dir, "precipitation.nc")
...
for (i in 27:nl) {
  ...
  df <- data.frame(munic, amc_1980, pre, date)

  # rename variables
  colnames(df)[3] <- "monthly_pre"

  # save data as .csv
  setwd(out_dir)
  write.csv(df,
    paste0(out_dir, "/amc_pre_csv/", date, "_amc_pre.csv"),
    row.names = TRUE,
  ) # overwrites
